...
export default function Prizes({ title = 'Prizes', items = [], total = 2000000, cta = { label: 'Register now', href: '#register' } }) {
  ...
  return (
    <section id="prizes" className="bg-[color:var(--bg-base,#0A0B0E)]">
      <div className="mx-auto max-w-6xl px-4 py-16">
        ...
        <p className="mt-3 text-center text-3xl font-extrabold text-[color:var(--accent-orange,#FF6A3D)]">{formatINR(animatedTotal)}</p>
        ...
        <div className="mt-8 text-center">
          <a href={cta.href} aria-label={cta.label} className="inline-flex items-center justify-center rounded-md bg-[color:var(--cta-bg,#FF6A3D)] px-5 py-3 font-medium text-[color:var(--cta-fg,#0B0F14)] hover:bg-[color:var(--cta-hover,#FF835C)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[rgba(182,255,77,0.35)] ring-offset-[color:var(--bg-base,#0A0B0E)]">{cta.label}</a>
        </div>
      </div>
    </section>
  )
