...
export function PrimaryButton({ as = 'a', href, children, className = '', ...rest }) {
  ...
  return (
    <Comp
      ...
      className={`inline-flex items-center justify-center rounded-md bg-[color:var(--cta-bg,#FF6A3D)] bg-gradient-to-r from-[color:var(--accent-orange,#FF6A3D)] to-[color:var(--accent-magenta,#FF4DD8)] px-5 py-3 font-medium text-[color:var(--cta-fg,#0B0F14)] shadow-[0_8px_30px_rgba(255,106,61,0.35)] hover:brightness-[1.05] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[rgba(0,229,255,0.35)] ring-offset-[color:var(--bg-base,#0A0B0E)] ${className}`}
      {...rest}
    >
      {children}
    </Comp>
